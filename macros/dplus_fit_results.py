...
for plot_type, theory_dict in THEORY_DICT.items():
    ...
    for obs_name, obs in OBSERVABLES.items():
        ...
        priors = {
            "Wminus": h_minus.Integral() / 2.,
            "Wminus_1": h_minus.GetBinContent(1) / 2.,
            "Wminus_2": h_minus.GetBinContent(2) / 2.,
            "Wminus_3": h_minus.GetBinContent(3) / 2.,
            "Wminus_4": h_minus.GetBinContent(4) / 2.,
            "Wminus_5": h_minus.GetBinContent(5) / 2.,
            "Wplus": h_plus.Integral() / 2.,
            "Wplus_1": h_plus.GetBinContent(1) / 2.,
            "Wplus_2": h_plus.GetBinContent(2) / 2.,
            "Wplus_3": h_plus.GetBinContent(3) / 2.,
            "Wplus_4": h_plus.GetBinContent(4) / 2.,
            "Wplus_5": h_plus.GetBinContent(5) / 2.,
        }
        ...
        for lep in ["minus", "plus"]:
            ...
            for prediction, prediction_dict in theory_dict.items():
                ...
                if 'QCDScaleFile' not in prediction_dict:
                    ...
                    gr_qcd = f_qcd.Get(f"mu_{lep}_Dplus_{name}__fractionalErr_cross")
                    gr_qcd_norm = f_qcd.Get(f"mu_{lep}_Dplus_{name}__fractionalErr_norm")
                    assert gr_qcd, (f"mu_{lep}_Dplus_{name}__fractionalErr_cross", prediction)
                else:
                    f_qcd = ROOT.TFile(os.path.join(DIR_THEORY, f"{prediction_dict['QCDScaleFile']}_{name}_W{lep}D{meson_charge}.root"))
                    # Sherpa has incorrect histogram name
                    if "Sherpa" in prediction:
                        gr_qcd = f_qcd.Get(f"mu_{meson_charge}_Dplus_{name}__fractionalErr_cross")
                        gr_qcd_norm = f_qcd.Get(f"mu_{meson_charge}_Dplus_{name}__fractionalErr_norm")
                        assert gr_qcd, f"mu_{meson_charge}_Dplus_{name}__fractionalErr_cross"
                    else:
                        gr_qcd = f_qcd.Get(f"mu_{lep}_Dplus_{name}__fractionalErr_cross")
                        gr_qcd_norm = f_qcd.Get(f"mu_{lep}_Dplus_{name}__fractionalErr_norm")
                        assert gr_qcd, f"mu_{lep}_Dplus_{name}__fractionalErr_cross"
                ...
                if "histoName" in prediction_dict:
                    gr_theory = f_theory.Get(f"mu_{lep}_Dplus_{name}_{prediction_dict['histoName']}_cross")
                    gr_theory_norm = f_theory.Get(f"mu_{lep}_Dplus_{name}_{prediction_dict['histoName']}_norm")
                    assert gr_theory, f"mu_{lep}_Dplus_{name}_{prediction_dict['histoName']}_cross"
                else:
                    gr_theory = f_theory.Get(f"mu_{lep}_Dplus_{name}_{prediction}_cross")
                    gr_theory_norm = f_theory.Get(f"mu_{lep}_Dplus_{name}_{prediction}_norm")
                    assert gr_theory, f"mu_{lep}_Dplus_{name}_{prediction}_cross"
                ...
                mg_obs_ratio_theory.Add(gr_theory_ratio, "pe5")
