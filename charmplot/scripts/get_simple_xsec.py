...
def main(options):
    ...
    f_m = ROOT.TFile(options.mc, "READ")
    f_t = ROOT.TFile(options.truth, "READ")
    logging.info(f"read input files {f_d} {f_m} {f_t}")
    ...
    h_inclusive_m = ROOT.TH1F("h_inclusive_m", "h_inclusive_m", len(channels), 0, len(channels))
    ...
    for i, c in enumerate(channels):
        ...
        h_inclusive_m.GetXaxis().SetBinLabel(i + 1, c)
    ...
    for var in variables:
        ...
        for i, c in enumerate(channels):
            ...
            h_m = f_m.Get(f"{reco_string}/{reco_string}__{var.name}").Clone(f"h_m_{reco_string}")
            h_t = f_t.Get(f"{c}/{c}__{var.name}").Clone(f"h_t_{reco_string}")
            logging.info(f"read histograms {h_d} {h_m} {h_t}")
            ...
            # make legend
            canv.make_legend(gr_data_stat_err, [gr_mc_stat_err], samples, data_name="Sherpa (MG Unf.)")
