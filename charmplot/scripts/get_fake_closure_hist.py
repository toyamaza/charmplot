...
def main(options, args):
    ...
    for l in leptons:
        for t in btags:
            ...
            for y in years:
                if first_year:
                    hist_data_minus_prompt = f.Get(f"Data_{y}_{l}_QCD_{t}_{dspecies}_{var}").Clone(f"DataMinusPrompt_{l}_QCD_{t}_{dspecies}_{var}")
                    hist_data_minus_prompt.Sumw2()
                    hist_mj = f.Get(f"Multijet_MatrixMethod_{y}_{l}_QCD_{t}_{dspecies}_{var}").Clone(f"Multijet_MatrixMethod_{l}_QCD_{t}_{dspecies}_{var}")
                    hist_mj.Sumw2()
                    first_year = False
                else:
                    hist_tmp = f.Get(f"Data_{y}_{l}_QCD_{t}_{dspecies}_{var}")
                    hist_data_minus_prompt.Add(hist_tmp,1)
                    hist_tmp2 = f.Get(f"Multijet_MatrixMethod_{y}_{l}_QCD_{t}_{dspecies}_{var}")
                    hist_mj.Add(hist_tmp2)
                for c in prompt_channels:
                    hist_tmp3 = f.Get(f"{c}_{y}_{l}_QCD_{t}_{dspecies}_{var}")
            ...
            #MET RW histograms
            ratio = hist_data_minus_prompt.Clone(f"RW_ratio_{l}_QCD_{t}_{dspecies}_{var}")
            ratio.Sumw2()
            ratio.Divide(hist_mj)
            ratio.Write()
            make_ratio_range(hist_data_minus_prompt, hist_mj,options.var_rw_range, f"RW_ratio_{l}_QCD_{t}_{dspecies}_{var}").Write()
